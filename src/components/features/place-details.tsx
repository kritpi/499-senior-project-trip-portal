...
// Helper to check if place is open without triggering deprecation warning
function safelyCheckIsOpen(
  ...
  try {
    return openingHours.isOpen() ?? null;
  } catch {
    return null;
  }
}
...
export function PlaceDetails({ place, onAdd, onClose }: PlaceDetailsProps) {
  const isOpen = safelyCheckIsOpen(place.opening_hours);
  const mainPhotoUrl =
      ...
      <CardHeader className="pb-2">
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {place.types?.slice(0, 3).map((type) => (
            <span
              key={type}
              className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-full text-[10px] capitalize text-muted-foreground"
            >
              {type.replace(/_/g, " ")}
            </span>
          ))}
            ...
            {place.website && (
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <a
                  ...
                  className="hover:underline text-primary truncate max-w-[250px]"
                >
                  Visit Website
                </a>
              </div>
            ...
            <div className="space-y-4">
              {place.reviews?.slice(0, 3).map((review, i) => (
                <div key={i} className="text-sm">
                  ...
                  <p className="text-muted-foreground text-xs line-clamp-3 italic">
                    "{review.text}"
                  </p>
                </div>
              ))}
